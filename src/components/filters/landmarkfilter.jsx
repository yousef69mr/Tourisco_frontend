...
export function filterLandmarks(landmarks, selectedCategory, searchQuery,
   activeOnly,ratingRange = null, searchQuerycategory) {


const groupedLandmarksByCategory = landmarks.reduce((acc, landmark) => {
const categoryLandmarks = acc[landmark.landmark.tourismCategoryObject] || [];
      return { ...acc, [landmark.landmark.tourismCategoryObject]: categoryLandmarks.concat(landmark) };
    }, {});
  
// Filter landmarks based on selected category
const filteredLandmarksByCategory = selectedCategory ?
     groupedLandmarksByCategory[selectedCategory] || [] : landmarks;
  ...
  const filteredLandmarksByActiveAndName = landmarks.filter(landmark => {
    return (!activeOnly || landmark.landmark.active) && (!searchQuery || landmark.title.toLowerCase().includes(searchQuery.toLowerCase()));
  });
  ...
  const filteredLandmarksByTypeCategory = filteredLandmarksByCategory.filter(landmark => {
    return (!searchQuerycategory || landmark.category_type.toLowerCase().includes(searchQuerycategory.toLowerCase()));
  });
