...
{console.log("governorates",governorates)}
  ...
  useEffect(() => {
    const filteredGovernorate = governorates.filter((governorate) => governorate.governorate.id === Specificgovernment?.governorate.id);
    if (filteredGovernorate.length > 0) {
      setSelectedgov(filteredGovernorate[0]);
    } else {
      setSelectedgov(null);
    }
  }, [governorates, Specificgovernment]);
  ...
  const handleLandMarkClick = (landmark) => {
    ...
    navigate(`/landmark/${landmark.id}`, { state: landmark });
  };
    ...
    {selectedgov ? (
      <>
          ...
          <div className={css.image_container}>
            <figure class={css.swing}>
              <div className={css.image}>
                <img src={backendBaseURL + selectedgov.governorate.emblem} alt={selectedgov.title} />
              </div>
            </figure>
          </div>
        </div>
        <div className={css.texts}>
          ...
          <p>{selectedgov.description} , <span>{t("GovernmentInfo.area")} {selectedgov.governorate.area} {t("GovernmentInfo.KM")} </span>
            , {selectedgov.governor} {t("GovernmentInfo.governor")} {selectedgov.title}</p>
        </div>
        <h1 className={css.box_title}>{t("GovernmentInfo.LandMarks Found in")}  {selectedgov.title} </h1>
        <div className={css.row3_container}>
          {landmarks.filter((landmark) => landmark.landmark.governorate.id === selectedgov.governorate.id).map((landmark) => (
            <div
              key={landmark.id}
              className={`${css.box5} ${css.grayishBlue} ${
                selectedlandmark?.id === landmark.landmark.id ? css.selected : ''
              }`}
             >
             ...
             { /*<div className={css.imgLandMarkContainer}>
              ...
              <p>{landmark?.description ? truncateText(landmark.description, 300) : ''}</p>
              <a  class={css.read_more}  onClick={() => handleLandMarkClick(landmark)}>  {t('GovernmentCards.Read More')} 
