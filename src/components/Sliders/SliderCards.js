...
const CalculateLandmarkLanguageData = (categories, governorates, landmark) => {
    const landmark_tourism_categories = landmark?.landmark?.tourism_categories?.map((category) => category?.id)
    const tourism_categories = categories?.filter((category) => landmark_tourism_categories?.includes(category?.category?.id))
